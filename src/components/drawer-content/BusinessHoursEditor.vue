...
const stored = computed(() => {
	const data = node.value?.data as
		| { times?: BusinessHourSlot[]; timezone?: string; connectors?: string[]; action?: string }
		| undefined
	return {
		times: data?.times ?? [],
...
<template>
	<div class="space-y-3">
		<header class="flex items-end justify-between gap-2">
			...
			<div>
				<label for="bh-tz" class="text-xs font-medium text-muted-foreground">Timezone</label>
				<input
					id="bh-tz"
		...
		<ul class="space-y-1">
			...
			>
				<label class="flex items-center gap-2 text-xs">
					<input v-model="draft.days[day.key].enabled" type="checkbox" class="accent-primary" />
		...
		<Button size="sm" class="w-full" @click="save">Save business hours</Button>
	</div>
</template>
